...
function modifyRequire(ctx, m, args) {
  ...
  //make
  var pkg = helper.resolveId(m.dir, target);

  var id = typeof pkg === 'string' ? pkg : pkg.id;

  var t = ctx.modules.get(id);

  if(!t) throw "missing module: " + id;
...
function getModuleObject(ctx, m) {
  ...
  if(m.params.__dirname || m.params.__filename)
    props.push(util.format('d: "%s"', path.relative(ctx.entryDir, m.dir)));
