...
module.exports = function(ctx) {
  ...
  function rewriteFile(id, file) {
    if(ctx.opts.compress && file.pkg.json.bale && file.ext === '.js') {
      file.contents = helper.uglify(file.contents, {
        compress: {
          unsafe: true,
          unused: true,
          cascade: true
  ...
  return Q.all(map(ctx.files, rewriteFile)).then(function() {
    return ctx;
  });
};
...
//     return getModuleString(ctx, m);
//   });

//   var output = mainTemplate.
//     replace('/*EN☃TRY*/', ctx.modules.get(ctx.pkg.id).hash).
...
//   //make
//   var pkg = helper.resolvePkg(m.dir, target);
//   var t = ctx.modules.get(pkg.id);

//   if(!t) throw "missing module '" + pkg.id + "' (from: " + m.id + ")";

//   //modify source to require the hash
//   arg.update('"'+t.hash+'"');
// }

// function getModuleString(ctx, m) {
...
//     props.push(util.format('d: "%s"', path.relative(m.dir, path.dirname(ctx.pkg.entry))));
